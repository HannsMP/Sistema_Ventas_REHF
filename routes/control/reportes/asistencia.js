...
module.exports = {
  load: true,
  route: "/control/reportes/asistencia",
  viewRenderPath: resolve('view', 'control', 'reportes', 'asistencia.ejs'),
  ...
  nodeRoute: function (node) {
    ...
    let readTable = async (tableReq, res) => {
      ...
      try {
        let data = this.model.tb_asistencias.readInParts(tableReq);
        let recordsFiltered = this.model.tb_asistencias.readInPartsCount(tableReq);
        let recordsTotal = this.model.tb_asistencias.readCount();
