...
module.exports = {
  load: false,
  route: "/api/productos/table/readAll",
  post: [
    async function (req, res, next) {
      try {
        ...
        let list = await this.model.tb_productos.readAllJoin();

        let prediccion = {}
        prediccion.precio_venta = this.neuralNetwork.precio_venta.toFunction.toString();

        res.status(200).json({ list, permisos, prediccion })
      } catch (e) {
