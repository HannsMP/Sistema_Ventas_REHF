...
module.exports = {
  load: true,
  route: "/api/bot/profile/logout",
  post: [
    async function (req, res, next) {
      try {
        let session = this.cache.apiKey.read(req.cookies.apiKey);

        let permisos = await this.model.tb_permisos.userPathDelete(session.usuario.id, module.exports.route);
        ...
        res.status(200).json({ stateCurrent: this.bot.state() });
